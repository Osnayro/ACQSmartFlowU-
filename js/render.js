...
// ============================================================
// SMARTFLOW RENDER v9.2.1 - 100% COBERTURA + ULTRA SEGURO
...
const SmartFlowRender = (function() {
    ...
    function deepDisposeGroup(group) {
        ...
        // 1. Recorrer y liberar buffers ANTES de desvincular
        ...
        // 2. Remover del grafo de escena de forma ultra segura (de atrás hacia adelante)
    ...
    function createTankVertical(eq) {
        ...
        var r = (eq.diametro || 3000) / 2, h = eq.altura || 6000, group = new THREE.Group();
        ...
        for (var y = h*0.2; y < h; y += h*0.25) {
            var ring = new THREE.Mesh(new THREE.TorusGeometry(r+10, 15, 8, 32), matRing);
    ...
    function createPipeMesh(line) {
        ...
        var color = getPipeColor(line.spec || line.material), radius = ((line.diameter||4)*25.4)/2;
    ...
    function createFitting(comp, pos3D, dirVec, size, compType, spec) {
        var type = (compType || comp.type || '').toUpperCase(), s = size||100;
    ...
    // ============ VÁLVULAS E INSTRUMENTOS (COMPLETO) ============
    ...
    function refreshAllDimensions() {
        ...
        (_core.getDb().lines||[]).forEach(function(line){
            ...
            if (pts.length>=2) for (var i=0; i<pts.length-1; i++)
                _dimensionGroup.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(pts[i].x,pts[i].y+500,pts[i].z),new THREE.Vector3(pts[i+1].x,pts[i+1].y+500,pts[i+1].z)]),new THREE.LineBasicMaterial({color:0xfacc15,transparent:true,opacity:0.6})));
        });
    }
    ...
    function refreshAllFlowArrows() {
        ...
        (_core.getDb().lines||[]).forEach(function(line){
            ...
            for (var i=0; i<pts.length-1; i++){
                var mid=new THREE.Vector3((pts[i].x+pts[i+1].x)/2,(pts[i].y+pts[i+1].y)/2+300,(pts[i].z+pts[i+1].z)/2);
                var dir=new THREE.Vector3(pts[i+1].x-pts[i].x,pts[i+1].y-pts[i].y,pts[i+1].z-pts[i].z).normalize();
                var cone=new THREE.Mesh(new THREE.ConeGeometry(200,400,6,6),new THREE.MeshStandardMaterial({color:0x00f2ff,emissive:0x003344}));
    ...
    function fitCameraToEquipments() {
        ...
        if (!has){ camera.position.set(15000,10000,15000); controls.target.set(0,0,0); controls.update(); return; }
    ...
    function init(coreInstance, engineInstance) {
        ...
        scheduleRefresh();
        console.log("✔ SmartFlowRender v9.2.1 - ULTRA SEGURO");
    }
